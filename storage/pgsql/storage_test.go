...
func createLogNodesForTreeAtSize(ts, rev int64) []storage.Node {
	...
	tree := merkle.NewCompactMerkleTree(th)
...
func openTestDBOrDie() *sql.DB {
	db, err := OpenDB("postgres://test:zaphod@localhost/test")
	if err != nil {
		panic(err)
	}
	return db
...
func createTestDB(db *sql.DB) {
	_, err := db.Exec("SELECT upsert_trees($1,$2,$3)", 23, 23, "MAP")
	if err != nil {
		panic(err)
	}
}
...
func prepareTestTreeDB(db *sql.DB, treeID int64, t *testing.T) {
	...
	for _, table := range allTables {
		_, err := db.Exec(fmt.Sprintf("DELETE FROM %s WHERE TreeId=$1", table), treeID)
...
func prepareTestLogDB(db *sql.DB, logID logIDAndTest, t *testing.T) {
	...
	_, err := db.Exec(`SELECT upsert_trees($1,$2,$3)`, logID.logID, logID.logID, "LOG")
