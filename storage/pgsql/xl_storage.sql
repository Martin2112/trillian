-- Postgres version of the tree schema. Note this is specific for Postgres XL.
-- It cannot be used on generic Postgres servers.
...
-- TODO(Martin2112): This distribution is weak because we can only distribute on a single
-- column that is part of the primary key. We could make code changes or use triggers to
...
) DISTRIBUTE BY HASH(SubTreeId);

-- The TreeRevisionIdx is used to enforce that there is only one STH at any
-- tree revision
-- TODO(Martin2112): Replication looks reasonable for this. Hash distribution is problematic
-- as it can't be done on the TreeHeadTimestamp. Review later.
...
CREATE TABLE IF NOT EXISTS LeafData(
  ...
  FOREIGN KEY(TreeId) REFERENCES Trees(TreeId) ON DELETE CASCADE
) DISTRIBUTE BY HASH(LeafIdentityHash);
...
-- TODO(Martin2112): Distribution by TreeId is terrible but XL limitations prevent use of other
-- columns without a redesign of the schema.
