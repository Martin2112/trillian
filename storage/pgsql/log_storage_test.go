...
func setAllowsDuplicates(db *sql.DB, treeID int64, allowDuplicates bool) error {
	stmt, err := db.Prepare("UPDATE Trees SET AllowsDuplicateLeaves = $1 WHERE TreeId = $2")
	if err != nil {
		return err
	}
	defer stmt.Close()
...
func TestQueueLeaves(t *testing.T) {
	...
	if err := DB.QueryRow("SELECT COUNT(*) FROM Unsequenced WHERE TreeID=$1", logID.logID).Scan(&count); err != nil {
