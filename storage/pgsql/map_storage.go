...
type pgSQLMapStorage struct {
	*pgSQLTreeStorage
}

// NewMapStorage creates a pgSQLMapStorage instance for the specified MySQL URL.
func NewMapStorage(db *sql.DB) (storage.MapStorage, error) {
	return &pgSQLMapStorage{
		pgSQLTreeStorage: newTreeStorage(db),
	}, nil
}

func (m *pgSQLMapStorage) CheckDatabaseAccessible(ctx context.Context) error {
...
func (m *pgSQLMapStorage) Snapshot(ctx context.Context) (storage.ReadOnlyMapTX, error) {
	tx, err := m.db.Begin()
	if err != nil {
		return nil, err
	}
...
func (m *pgSQLMapStorage) SnapshotForTree(ctx context.Context, treeID int64) (storage.ReadOnlyMapTreeTX, error) {
...
func (m *mapTreeTX) Get(revision int64, indexes [][]byte) ([]trillian.MapLeaf, error) {
	stmt, err := m.ms.getStmt(selectMapLeafSQL, 3, len(indexes))
	if err != nil {
		return nil, err
	}
	stx := m.tx.Stmt(stmt)
