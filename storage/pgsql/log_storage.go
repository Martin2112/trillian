...
func (m *pgSQLLogStorage) getLeavesByMerkleHashStmt(num int, orderBySequence bool) (*sql.Stmt, error) {
	if orderBySequence {
...
func (m *pgSQLLogStorage) getDeleteUnsequencedStmt(first, num int) (*sql.Stmt, error) {
	return m.getStmt(deleteUnsequencedSQL, first, num)
}
...
func (m *pgSQLLogStorage) Snapshot(ctx context.Context) (storage.ReadOnlyLogTX, error) {
	tx, err := m.db.Begin()
	if err != nil {
...
func (m *pgSQLLogStorage) beginInternal(ctx context.Context, treeID int64) (storage.LogTreeTX, error) {
...
func (m *pgSQLLogStorage) BeginForTree(ctx context.Context, treeID int64) (storage.LogTreeTX, error) {
	return m.beginInternal(ctx, treeID)
}

func (m *pgSQLLogStorage) SnapshotForTree(ctx context.Context, treeID int64) (storage.ReadOnlyLogTreeTX, error) {
...
func (t *logTreeTX) DequeueLeaves(limit int, cutoffTime time.Time) ([]trillian.LogLeaf, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to prepare dequeue select: %s", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to select rows for work: %s", err)
	}
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("Error scanning work rows: %s", err)
		}
	...
	if rows.Err() != nil {
		return nil, fmt.Errorf("row scan error: %v", rows.Err())
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to remove leaves: %v", err)
	}
...
func (t *logTreeTX) removeSequencedLeaves(leaves []trillian.LogLeaf) error {
	tmpl, err := t.ls.getDeleteUnsequencedStmt(2, len(leaves))
	if err != nil {
