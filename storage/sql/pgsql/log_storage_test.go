...
func createFakeLeaf(db *sql.DB, logID int64, rawHash, hash, data, extraData []byte, seq int64, t *testing.T) *trillian.LogLeaf {
	_, err := db.Exec("INSERT INTO LeafData(TreeId, LeafIdentityHash, LeafValue, ExtraData) VALUES($1,$2,$3,$4)", logID, rawHash, data, extraData)
	_, err2 := db.Exec("INSERT INTO SequencedLeafData(TreeId, SequenceNumber, LeafIdentityHash, MerkleLeafHash) VALUES($1,$2,$3,$4)", logID, seq, rawHash, hash)
...
func TestQueueDuplicateLeaf(t *testing.T) {
	...
	for ti, test := range tests {
		func() {
			...
			if err != nil {
				t.Fatalf("Failed to queue leaves @%d: %v", ti, err)
			}
			testonly.Commit(tx, t)

			if len(existing) != len(test.want) {
				t.Errorf("|QueueLeaves()|=%d @%d; want %d", len(existing), ti, len(test.want))
				return
			}
...
func TestQueueLeaves(t *testing.T) {
	...
	var count int
	if err := dbWrapper.DB().QueryRow("SELECT COUNT(*) FROM Unsequenced WHERE TreeID=$1", logID).Scan(&count); err != nil {
	...
	if err := dbWrapper.DB().QueryRow("SELECT DISTINCT QueueTimestampNanos FROM Unsequenced WHERE TreeID=$1", logID).Scan(&queueTimestamp); err != nil {
...
func TestGetLeavesByHash(t *testing.T) {
	...
	data := []byte("some data")
	createFakeLeaf(dbWrapper.DB(), logID, dummyRawHash, dummyHash, data, someExtraData, sequenceNumber, t)
...
func TestGetLeafDataByIdentityHash(t *testing.T) {
	...
	s := coresql.NewLogStorage(dbWrapper)
	data := []byte("some data")
	leaf := createFakeLeaf(dbWrapper.DB(), logID, dummyRawHash, dummyHash, data, someExtraData, sequenceNumber, t)
	leaf.LeafIndex = -1
	leaf.MerkleLeafHash = []byte(dummyMerkleLeafHash)
	leaf2 := createFakeLeaf(dbWrapper.DB(), logID, dummyHash2, dummyHash2, data, someExtraData, sequenceNumber+1, t)
	leaf2.LeafIndex = -1
	...
	for _, test := range tests {
		func() {
			...
			leaves, err := tx.GetLeafDataByIdentityHash(test.hashes)
			if err != nil {
...
func TestGetSequencedLeafCount(t *testing.T) {
...

	{
		// Create fake leaf as if it had been sequenced
		data := []byte("some data")
		createFakeLeaf(dbWrapper.DB(), logID1, dummyHash, dummyRawHash, data, someExtraData, sequenceNumber, t)

		// Create fake leaves for second tree as if they had been sequenced
		data2 := []byte("some data 2")
		data3 := []byte("some data 3")
		createFakeLeaf(dbWrapper.DB(), logID2, dummyHash2, dummyRawHash, data2, someExtraData, sequenceNumber, t)
		createFakeLeaf(dbWrapper.DB(), logID2, dummyHash3, dummyRawHash, data3, someExtraData, sequenceNumber+1, t)
	}
